...
get_templateconf()
{
	...
	local machine_path=$(find ${ROOTOE}/layers/meta-100ask-st/ -type d \( -name '.git' -o -name '.repo' -o -name 'build*' -o -name 'source*' -o -name 'script*' \) -prune -o -type f -name "$MACHINE.conf" | grep "/machine/$MACHINE.conf" | sed 's|\(.*\)/conf/machine/\(.*\)|\1|')
